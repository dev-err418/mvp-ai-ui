...
export const Graphs = () => {
    ...
    useEffect(() => {
        ...
        const graph4Total = graph1Total + graph2Total + graph3Total

        var graph4temp = []
        setNextProps([
            Number(graph1Total/graph4Total * 100).toFixed(2),
            Number(graph2Total/graph4Total * 100).toFixed(2),
            Number(graph2Total/graph4Total * 100).toFixed(2),
        ])
        graph4temp.push({
    ...
    return (
        <View style={{ padding: 20, width: "100vw", backgroundColor: "white" }}>
            ...
            />
            <TouchableOpacity
                onPress={() => navigation.navigate("EndAi", {data: nextProps})} 
                style={{
                    padding: 10,
                    borderRadius: 10,
