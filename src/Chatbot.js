...
const Message = ({ el, i, mess }) => {
  ...
  return (
    <Animated.View
      ...
      style={{ maxWidth: "80%", marginTop: 10, padding: el.type == 2 ? 0 : 10, borderRadius: 10, backgroundColor: el.type == 0 ? green : "green", alignSelf: el.type == 0 ? "flex-end" : "flex-start", transform: [{ scale: scale }] }}>
      {el.type == 2 && i == mess.length - 1 &&
        <View style={{ width: 50, height: 40, borderRadius: 10, backgroundColor: "green" }}>
          <TypingAnimation
            dotColor={"white"}
            dotMargin={5}
            dotAmplitude={3}
...
export const SearchArea = ({ chat, setChat, setShow }) => {
  ...
  return (
    ...
    shadowOpacity: 0.25,
    shadowRadius: 5, height: "100vh", width: "50vw", justifyContent: "center", alignItems: "center"}}>
